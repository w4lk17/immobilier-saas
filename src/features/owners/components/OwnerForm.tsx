...
import { ownerCreateSchema, OwnerFormData, OwnerUpdateFormData, ownerUpdateSchema } from '../schemas/ownerSchemas';
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Form, FormControl, FormDescription, FormField, FormItem, FormLabel, FormMessage } from "@/components/ui/form";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { FrontendOwner, FrontendUserSnippet } from '@/types'; // Importer FrontendOwner pour initialData
...
interface OwnerFormProps {
	initialData?: FrontendOwner | null; // Pour pré-remplir en mode édition
	onSubmit: (data: OwnerFormData | OwnerUpdateFormData) => Promise<void>;
	isLoading?: boolean;
...
export function OwnerForm({
	...
	onSubmit,
	isLoading = false,
	submitButtonText = initialData ? "Mettre à jour" : "Créer propriétaire",
	...
	const form = useForm<OwnerFormData | OwnerUpdateFormData>({
		resolver: zodResolver(isEditMode ? ownerUpdateSchema : ownerCreateSchema),
		defaultValues: isEditMode && initialData ? {
			// Pour la mise à jour, userId n'est généralement pas modifié via ce formulaire
			// On se base sur ownerUpdateSchema qui rend les champs optionnels
		...
		<>
			...
			<div>
				<Card className="max-w-2xl ">
					<CardHeader>
						<CardTitle>{isEditMode ? "Modifier le Profil Propriétaire" : "Nouveau Profil Propriétaire"}</CardTitle>
						<CardDescription>
							{isEditMode
								? `Modification du profil de ${initialData?.user?.firstName || ''} ${initialData?.user?.lastName || ''} (ID Propriétaire: ${initialData?.id})`
								: "Remplissez les informations pour créer un nouveau profil propriétaire."}
						</CardDescription>
					</CardHeader>
					<CardContent>
						<Form {...form}>
									...
									<FormField
										...
										render={({ field }) => (
											...
											/> */}
												...
												<FormDescription>
													Le compte utilisateur existant (créé via la section Utilisateurs) à lier à ce profil propriétaire.
												</FormDescription>
												<FormMessage />
											</FormItem>
										)}
