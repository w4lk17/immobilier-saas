...
import { tenantCreateSchema, TenantFormData, TenantUpdateFormData, tenantUpdateSchema } from '../schemas/tenantSchemas';
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Form, FormControl, FormDescription, FormField, FormItem, FormLabel, FormMessage } from "@/components/ui/form";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { FrontendTenant, FrontendUserSnippet } from '@/types'; // Importer FrontendTenant pour initialData
...
interface TenantFormProps {
	initialData?: FrontendTenant | null; // Pour pré-remplir en mode édition
	onSubmit: (data: TenantFormData | TenantUpdateFormData) => Promise<void>;
	isLoading?: boolean;
...
export function TenantForm({
	...
	onSubmit,
	isLoading = false,
	submitButtonText = initialData ? "Mettre à jour" : "Créer locataire",
	...
	const form = useForm<TenantFormData | TenantUpdateFormData>({
		...
		defaultValues: isEditMode && initialData ? {
			...
			// On se base sur tenantUpdateSchema qui rend les champs optionnels
		...
		<>
						...
						<div className='w-full xl:w-3/4 space-y-6'>
							...
							<div className='bg-primary-foreground p-4 rounded-lg space-y-4'>
								...
								<div className='flex items-center space-x-2'>
									...
									<Label htmlFor="compte" className='text-sm font-medium'>Compte Entreprise</Label>
								</div>
			...
			<div>
				<Card className="max-w-2xl ">
					<CardHeader>
						<CardTitle>{isEditMode ? "Modifier le Profil Locataire" : "Nouveau Profil Locataire"}</CardTitle>
						<CardDescription>
							{isEditMode
								? `Modification du profil de ${initialData?.user?.firstName || ''} ${initialData?.user?.lastName || ''} (ID Locataire: ${initialData?.id})`
								: "Remplissez les informations pour créer un nouveau profil locataire."}
						</CardDescription>
					</CardHeader>
					<CardContent>
						<Form {...form}>
									...
									<FormField
										...
										render={({ field }) => (
											...
											/> */}
												...
												<FormDescription>
													Le compte utilisateur existant (créé via la section Utilisateurs) à lier à ce profil locataire.
												</FormDescription>
												<FormMessage />
											</FormItem>
										)}
